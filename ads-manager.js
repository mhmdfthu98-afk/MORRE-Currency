...
class AdsManager {
    ...
    async init() {
        try {
            ...
            console.log('✅ Ads Manager initialized (Environment:', this.currentEnv, ')');
            return true;
        } catch (error) {
    ...
    getDailyCount() {
        ...
        const stored = localStorage.getItem('morre_ads_daily');
        if (stored) {
            try {
                ...
                console.warn('⚠️ Could not parse daily count:', e);
            }
        }
        return 0;
    }
    ...
    saveDailyCount(count) {
        const today = new Date().toDateString();
        localStorage.setItem('morre_ads_daily', JSON.stringify({
            date: today,
            count: count
        }));
    }
